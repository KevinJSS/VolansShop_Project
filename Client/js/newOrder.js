...
function newOrder() {
    window.alert('Orden de compra realizada correctamente.');
}
...
function ifCurrentSuppliesEmpty() {
    if (supplyCounter === 0) {
        divCurrentSupplies.innerHTML = "<p style='color: #228ddb;'>A&uacute;n no hay suministros agregados a la orden de compra</p>";
    }
}
ifCurrentSuppliesEmpty();
...
document.getElementById('add-order').addEventListener('click', e => {
    if (supplyCounter !== 0) {
        ...
        window.alert('No se han agregado suministros a la orden de compra');
    }
});
